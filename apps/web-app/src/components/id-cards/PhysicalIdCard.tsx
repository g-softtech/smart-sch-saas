...
export default function PhysicalIdCard({ student, school, qrToken }: PhysicalIdCardProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-6 print:block">
      {/* FRONT OF CARD */}
      <div 
        className="relative overflow-hidden bg-white border border-gray-300 shadow-md print:shadow-none print:border-none rounded-xl print:rounded-none flex flex-col"
        style={{
          width: '85.6mm',
          height: '54mm',
      ...
      >
        ...
        >
          {school.logoUrl ? (
            ...
            <img src={school.logoUrl} alt="School Logo" className="h-6 max-w-full object-contain mr-2" />
          ) : (
            ...
            <div className="w-full aspect-[3/4] bg-gray-100 border border-gray-200 rounded overflow-hidden flex items-center justify-center relative">
              {student.photoUrl ? (
                ...
                <div className="text-center text-gray-400 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[7px] uppercase font-semibold">No Photo</span>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="mt-1 space-y-0.5">
                <div className="text-[9px]">
                  ...
                  <span className="font-semibold text-gray-800">{student.className || <span className="text-gray-400 italic">Unassigned</span>}</span>
                </div>
                {student.armName && (
                  <div className="text-[9px]">
                    <span className="text-gray-500 font-medium">Arm: </span>
                    <span className="font-semibold text-gray-800">{student.armName}</span>
                  </div>
                )}
                <div className="text-[9px]">
                  <span className="text-gray-500 font-medium">Role: </span>
                  <span className="font-semibold text-gray-800 uppercase">Student</span>
                </div>
              </div>
            </div>
      ...
      {/* BACK OF CARD */}
      <div 
        className="relative overflow-hidden bg-white border border-gray-300 shadow-md print:shadow-none print:border-none rounded-xl print:rounded-none flex flex-col print:mt-4"
        style={{
          width: '85.6mm',
          height: '54mm',
          printColorAdjust: 'exact',
        ...
        <div className="flex-1 p-4 flex flex-col">
          <div className="mb-2">
            ...
            <ul className="text-[8px] text-gray-600 space-y-1 list-disc pl-3">
              ...
              <li>Present this card for scanning during arrival, departure, and designated academic activities.</li>
              <li>Loss of this card must be reported immediately to the school administration.</li>
            </ul>
          </div>
          
          <div className="mt-auto pt-2 border-t border-gray-200 text-center">
            <p className="text-[8px] font-bold text-gray-800">If found, please return to:</p>
            <p className="text-[9px] font-bold text-gray-900 mt-0.5">{school.name}</p>
            {school.address ? (
              <p className="text-[8px] text-gray-600 mt-0.5">{school.address}</p>
            ) : (
              <p className="text-[8px] text-gray-400 italic mt-0.5">[Address not configured]</p>
            )}
            {school.motto && (
              <p className="text-[7px] italic text-gray-500 mt-1">&quot;{school.motto}&quot;</p>
            )}
          </div>
        </div>
      </div>
