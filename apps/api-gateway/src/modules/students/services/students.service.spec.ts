...
describe("StudentsService (Unit / Mocked)", () => {
  ...
  describe("createStudent", () => {
    ...
    it(
      "creates student successfully when school belongs to active tenant",
      withTenant(async () => {
        repo.findSchool.mockResolvedValueOnce({
          id: SCHOOL_ID,
          tenantId: TENANT_ID,
        } as any);
        ...
        expect(repo.mintStudentNumber).toHaveBeenCalledWith(
          TENANT_ID,
          SCHOOL_ID,
          undefined,
        );
    ...
    it(
      "rejects when school not found (cross-tenant or nonexistent)",
      withTenant(async () => {
        // Kernel returns null for cross-tenant records.
        repo.findSchool.mockResolvedValueOnce(null);

        await expect(service.createStudent(baseInput)).rejects.toThrow(
          BadRequestException,
        );
      }),
    );
    ...
    it(
      "rejects when school tenantId does not match WorkspaceContext tenantId",
      withTenant(async () => {
        repo.findSchool.mockResolvedValueOnce({
          id: SCHOOL_ID,
    ...
    it(
      "calls mintStudentNumber with correct tenantId and schoolId",
      withTenant(async () => {
        repo.findSchool.mockResolvedValueOnce({
          id: SCHOOL_ID,
          tenantId: TENANT_ID,
  ...
  describe("createGuardian", () => {
    it(
      "creates guardian scoped to active tenant",
      withTenant(async () => {
        repo.createGuardian.mockResolvedValueOnce({ id: "g-1" } as any);

        const result = await service.createGuardian({
          firstName: "Ola",
          lastName: "Ade",
        });
  ...
  describe("linkGuardian", () => {
    ...
    it(
      "links guardian to student when both belong to active tenant",
      withTenant(async () => {
        ...
        repo.findGuardian.mockResolvedValueOnce({
          id: "g-1",
          tenantId: TENANT_ID,
        } as any);
        repo.findStudentGuardianLink.mockResolvedValueOnce(null);
        repo.createStudentGuardianLink.mockResolvedValueOnce({
          id: "sg-1",
        } as any);
    ...
    it(
      "rejects when student not found (cross-tenant)",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(null);

        await expect(service.linkGuardian(linkInput)).rejects.toThrow(
          BadRequestException,
        );
      }),
    );
    ...
    it(
      "rejects when guardian not found (cross-tenant)",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce({
          id: "stu-1",
          tenantId: TENANT_ID,
    ...
    it(
      "rejects duplicate guardian link",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce({
          id: "stu-1",
          tenantId: TENANT_ID,
        ...
        repo.findStudentGuardianLink.mockResolvedValueOnce({
          id: "link-1",
        } as any);
    ...
    it(
      "clears existing primary guardian before setting new primary",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce({
          id: "stu-1",
          tenantId: TENANT_ID,
        ...
        expect(repo.clearPrimaryGuardian).toHaveBeenCalledWith(
          "stu-1",
          expect.anything(),
        );
      }),
    ...
    it(
      "does not clear primary when isPrimary is false",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce({
          id: "stu-1",
          tenantId: TENANT_ID,
    ...
    it(
      "correctly records isEmergencyContact separately from relationship",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce({
          id: "stu-1",
          tenantId: TENANT_ID,
  ...
  describe("createEnrollment", () => {
    ...
    it(
      "enrolls student successfully when all parents are valid",
      withTenant(async () => {
    ...
    it(
      ...
      withTenant(async () => {
        ...
        await expect(service.createEnrollment(enrollInput)).rejects.toThrow(
          BadRequestException,
        );
      }),
    );
    ...
    it(
      "rejects enrollment for non-ACTIVE student",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce({
          ...activeStudent,
          status: "WITHDRAWN",
    ...
    it(
      "rejects when AcademicYear not found (cross-tenant)",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(activeStudent as any);
    ...
    it(
      "rejects when AcademicYear belongs to a different school than student",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(activeStudent as any);
    ...
    it(
      "rejects when Class belongs to a different school than student",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(activeStudent as any);
        repo.findAcademicYear.mockResolvedValueOnce(academicYear as any);
    ...
    it(
      "rejects when Arm does not belong to the specified Class",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(activeStudent as any);
        repo.findAcademicYear.mockResolvedValueOnce(academicYear as any);
        repo.findClass.mockResolvedValueOnce(classEntity as any);
        repo.findArm.mockResolvedValueOnce({
          id: "arm-1",
          classId: "class-OTHER",
        } as any);

        await expect(
          service.createEnrollment({ ...enrollInput, armId: "arm-1" }),
        ).rejects.toThrow(BadRequestException);
      }),
    );

    it(
      "rejects when Arm not found (cross-tenant)",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(activeStudent as any);
        repo.findAcademicYear.mockResolvedValueOnce(academicYear as any);
        repo.findClass.mockResolvedValueOnce(classEntity as any);
    ...
    it(
      "rejects duplicate ACTIVE enrollment for same student/year (pre-check)",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(activeStudent as any);
        repo.findAcademicYear.mockResolvedValueOnce(academicYear as any);
        repo.findClass.mockResolvedValueOnce(classEntity as any);
        repo.findActiveEnrollment.mockResolvedValueOnce({
          id: "existing-enr",
        } as any);
    ...
    it(
      "maps Prisma P2002 unique violation to ConflictException (race condition)",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(activeStudent as any);
        repo.findAcademicYear.mockResolvedValueOnce(academicYear as any);
        repo.findClass.mockResolvedValueOnce(classEntity as any);
  ...
  describe("transferEnrollment", () => {
    ...
    it(
      "transfers enrollment and preserves old record",
      withTenant(async () => {
        ...
        repo.transferEnrollment.mockResolvedValueOnce({
          id: "enr-2",
          status: "ACTIVE",
        } as any);
    ...
    it(
      "rejects transfer of a non-ACTIVE enrollment",
      withTenant(async () => {
        repo.findEnrollment.mockResolvedValueOnce({
          ...activeEnrollment,
          status: "WITHDRAWN",
        } as any);
    ...
    it(
      "rejects when target class belongs to a different school",
      withTenant(async () => {
        repo.findEnrollment.mockResolvedValueOnce(activeEnrollment as any);
        repo.findClass.mockResolvedValueOnce({
          id: "class-2",
    ...
    it(
      "student status remains ACTIVE after an internal transfer (not TRANSFERRED)",
      withTenant(async () => {
        repo.findEnrollment.mockResolvedValueOnce(activeEnrollment as any);
        repo.findClass.mockResolvedValueOnce({
          id: "class-2",
    ...
    it(
      "rejects when target arm does not belong to target class",
      withTenant(async () => {
        repo.findEnrollment.mockResolvedValueOnce(activeEnrollment as any);
        repo.findClass.mockResolvedValueOnce({
          id: "class-2",
  ...
  describe("withdrawStudent", () => {
    ...
    it(
      "withdraws student and sets status to WITHDRAWN",
      withTenant(async () => {
        repo.findEnrollment.mockResolvedValueOnce(activeEnrollment as any);
        repo.findStudent.mockResolvedValueOnce({
          id: "stu-1",
        ...
        expect(repo.withdrawEnrollment).toHaveBeenCalledWith(
          "enr-1",
          undefined,
        );
        expect(repo.setStudentStatus).toHaveBeenCalledWith(
          "stu-1",
          "WITHDRAWN",
        );
      }),
    ...
    it(
      "rejects withdrawal of a non-ACTIVE enrollment",
      withTenant(async () => {
        repo.findEnrollment.mockResolvedValueOnce({
          ...activeEnrollment,
          status: "TRANSFERRED",
    ...
    it(
      "rejects when enrollment does not belong to the specified student",
      withTenant(async () => {
        repo.findEnrollment.mockResolvedValueOnce({
          ...activeEnrollment,
          studentId: "stu-OTHER",
    ...
    it(
      "historical enrollment row is preserved (withdrawEnrollment updates, not deletes)",
      withTenant(async () => {
        repo.findEnrollment.mockResolvedValueOnce(activeEnrollment as any);
        repo.findStudent.mockResolvedValueOnce({
          id: "stu-1",
        ...
        repo.withdrawEnrollment.mockResolvedValueOnce({
          id: "enr-1",
          status: "WITHDRAWN",
        } as any);
        repo.setStudentStatus.mockResolvedValueOnce({} as any);
  ...
  describe("read operations", () => {
    it(
      "getStudent rejects when student not found (cross-tenant)",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(null);
        await expect(service.getStudent("stu-x")).rejects.toThrow(
          BadRequestException,
        );
      }),
    );
    ...
    it(
      "listEnrollments rejects when student not found (cross-tenant)",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(null);
        await expect(service.listEnrollments("stu-x")).rejects.toThrow(
          BadRequestException,
        );
      }),
    );
    ...
    it(
      "listStudentGuardians rejects when student not found (cross-tenant)",
      withTenant(async () => {
        repo.findStudent.mockResolvedValueOnce(null);
        await expect(service.listStudentGuardians("stu-x")).rejects.toThrow(
          BadRequestException,
        );
      }),
    );
