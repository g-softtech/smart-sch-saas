...
describe('StudentsController Security', () => {
  ...
  describe('createStudent schoolId security', () => {
    ...
    it('valid workspace -> student creation succeeds using workspace schoolId', async () => {
      ...
      const result = await controller.createStudent(req, validDto as CreateStudentDto);
      
      expect(result.success).toBe(true);
      expect(service.createStudent).toHaveBeenCalledWith(expect.objectContaining({
        schoolId: 'school-1',
        firstName: 'Ada',
      }));
    });
    ...
    it('client-supplied schoolId cannot override workspace schoolId', async () => {
      const req = { workspace: { tenantId: 'tenant-1', schoolId: 'school-workspace' } };
    ...
    it('missing/invalid workspace school context is rejected appropriately', async () => {
      ...
      await expect(controller.createStudent(req, validDto as CreateStudentDto))
        .rejects
