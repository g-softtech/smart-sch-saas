...
export class PoliciesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!request.workspace) {
      const membership = await tenantContext.run({ tenantId }, () => this.membershipRepo.findByUserId(request.user.sub, tenantId));
      if (!membership) {
    ...
    const userPermissions = (role as any).permissions.map((rp: any) => rp.permission.name);
