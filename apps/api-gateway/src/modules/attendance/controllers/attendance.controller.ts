import { Controller, Post, Get, Patch, Body, Param, Query, UseGuards, UseInterceptors, Req, BadRequestException, HttpCode } from '@nestjs/common';
...
export class AttendanceController {
  ...
  @Post('registers/bulk')
  @ApiOperation({ summary: 'Bulk create/upsert an attendance register for a class or arm' })
  ...
  async bulkCreateRegister(@Req() req: any, @Body() dto: BulkCreateAttendanceRegisterDto) {
    ...
    const register = await this.attendanceService.bulkCreateRegister(tenantId, schoolId, userId, dto);
    return AttendanceRegisterResponseDto.fromEntity(register);
  }

  @Get('registers')
  @ApiOperation({ summary: 'List attendance registers within the active workspace with pagination and date filtering' })
  @ApiResponse({ status: 200, type: [AttendanceRegisterResponseDto] })
  async getRegisters(@Req() req: any, @Query() query: AttendanceFilterQueryDto) {
    const { tenantId, schoolId } = req.workspace;
    if (!schoolId) throw new BadRequestException('School context is required');
  ...
  async getRegister(@Req() req: any, @Param('id') id: string) {
    const { tenantId, schoolId } = req.workspace;
    if (!schoolId) throw new BadRequestException('School context is required');
    const register = await this.attendanceService.getRegisterById(tenantId, schoolId, id);
    return AttendanceRegisterResponseDto.fromEntity(register);
  }

  @Patch('registers/:id/finalize')
  @HttpCode(200)
  @ApiOperation({ summary: 'Finalize an attendance register, locking it and emitting events' })
  @ApiResponse({ status: 200, type: AttendanceRegisterResponseDto })
  async finalizeRegister(@Req() req: any, @Param('id') id: string) {
    ...
    if (!schoolId) throw new BadRequestException('School context is required');
    const register = await this.attendanceService.finalizeRegister(tenantId, schoolId, id, userId, correlationId);
    return AttendanceRegisterResponseDto.fromEntity(register);
  }
  ...
  async getStudentAttendance(
    ...
    return records.map(r => {
      ...
      if (r.register) {
        (dto as any).register = AttendanceRegisterResponseDto.fromEntity(r.register);
      }
      return dto;
    });
  }
