...
export class ResendEmailAdapter implements EmailProvider {
  ...
  constructor() {
    ...
    if (!apiKey) {
      this.logger.warn('RESEND_API_KEY is not set. Email delivery will be skipped or fail.');
    }
    this.resend = new Resend(apiKey || 'unconfigured');
  }

  async sendEmail(options: EmailOptions): Promise<void> {
    const defaultFrom = process.env.EMAIL_FROM_ADDRESS || 'Acme Admissions <onboarding@resend.dev>';
