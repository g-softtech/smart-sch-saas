...
export class AdmissionsNotificationHandler {
  ...
  async handleApplicationSubmitted(event: DomainEvent) {
    this.logger.log(`Handling ApplicationSubmitted for aggregate: ${event.aggregateId}`);
    const payload = event.payload as any;
    
    if (!payload.email) {
      this.logger.warn(`No email found for applicant ${payload.applicantId}, skipping notification.`);
      return;
    }
  ...
  async handleStageChanged(event: DomainEvent) {
    this.logger.log(`Handling ApplicationStageChanged for aggregate: ${event.aggregateId}`);
    const payload = event.payload as any;
  ...
  async handleDecisionMade(event: DomainEvent) {
    this.logger.log(`Handling ApplicationDecisionMade for aggregate: ${event.aggregateId}`);
    const payload = event.payload as any;
    
    if (!payload.email) return;

    const isAccepted = payload.decision === 'APPROVED' || payload.decision === 'ENROLLED';
