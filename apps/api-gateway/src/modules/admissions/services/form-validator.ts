  ...
  static validate(schema: Record<string, FieldSchema> = {}, formData: any) {
    ...
    for (const key of dataKeys) {
      ...
      if (['firstName', 'lastName', 'dateOfBirth', 'gender'].includes(key)) {
         throw new BadRequestException(`Canonical applicant field '${key}' is not allowed in dynamic formData`);
      }
      ...
      switch (fieldDef.type) {
        case 'string':
          ...
          if (fieldDef.maxLength && value.length > fieldDef.maxLength) {
            throw new BadRequestException(`Field ${key} exceeds maximum length of ${fieldDef.maxLength}`);
          }
          if (fieldDef.minLength && value.length < fieldDef.minLength) {
            throw new BadRequestException(`Field ${key} does not meet minimum length of ${fieldDef.minLength}`);
          }
          break;
        case 'number':
        ...
        case 'select':
          ...
          if (typeof value !== 'string' || !fieldDef.options?.includes(value)) {
            throw new BadRequestException(`Field ${key} has an invalid select option. Allowed: ${fieldDef.options?.join(', ')}`);
          }
          break;
        case 'file':
          ...
          if (typeof value !== 'string') {
            throw new BadRequestException(`Field ${key} (file) must be a stable internal object key string. Raw binary is rejected.`);
          }
          if (value.startsWith('http://') || value.startsWith('https://')) {
            throw new BadRequestException(`Field ${key} (file) cannot be a URL. Expiring/presigned URLs are rejected.`);
          }
          ...
          break;
        default:
          throw new BadRequestException(`Unsupported field type '${fieldDef.type}' in schema for ${key}`);
      }
    }
  }
}
