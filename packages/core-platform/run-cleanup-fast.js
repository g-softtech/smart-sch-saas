...
async function runCleanup() {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      // --- PRE-DELETE ASSERTIONS (Batched for speed) ---
      const [tenant, campuses, schools, roles, membershipCount] = await Promise.all([
        tx.tenant.findUnique({ where: { id: tenantId } }),
        tx.campus.findMany({ where: { tenantId } }),
        tx.school.findMany({ where: { tenantId } }),
      ...
      // --- POST-DELETE VERIFICATION (Batched) ---
      const [postTenant, postCampus, postSchool, postRole] = await Promise.all([
        tx.tenant.count({ where: { id: tenantId } }),
        tx.campus.count({ where: { tenantId } }),
